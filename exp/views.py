...
from exp.models import ClassMetricRaw, Clustering, ClusteringMetric, ClusteringNormalize, MetricNormalize, MsInteractions, Project, S101MetricRaw, ScoringFinaleMedian, ScoringMedian
...
def index(request):
    ...
    return render(request, 'exp/index.html', data)
...
def project_assign(request, id):
    ...
    return render(request, 'exp/project_assign.html', data)
...
def project_cluster(request, id):
    ...
    return redirect('exp_project_assign', id=id)
...
def generate_ms_radar_chart(request, project_id):
    ...
    for i in range(ms_len):
        ...
        filename = 'exp_ms_radar_' + str(project_id) + '_' + str(i)
    ...
    return render(request, 'exp/ms_radar_chart.html', data)
